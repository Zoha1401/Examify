...
const AddProgrammingQuestion = () => {
  ...
  const handleAddProgrammingQuestion=async(e)=>{
    ...
    console.log("Details sent to API:", { ...progQuestion, testCases });
       try{
        const response=await axiosInstance.post(`/programmingQuestion/addProgrammingQuestion?examId=${examId}`,
          {
            ...progQuestion,
            testCases
          },
   ...
   <>
     <Button variant="primary">
      ...
      <div className="flex min-h-full flex-1 flex-col justify-center px-6 py-12 lg:px-8">
        ...
        <form onSubmit={handleAddProgrammingQuestion} className="space-y-6 mt-10">
          <div>
          ...
          <Dropdown>
            ...
            <Dropdown.Menu>
              {["Easy", "Medium", "Hard"].map((level) => (
                <Dropdown.Item
                  key={level}
                  onClick={() => setProgQuestion({ ...progQuestion, difficulty: level })}
                >
                  {level}
                </Dropdown.Item>
              ))}
