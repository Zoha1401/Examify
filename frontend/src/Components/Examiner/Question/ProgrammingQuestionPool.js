...
const ProgrammingQuestionPool = () => {
  ...
  useEffect(() => {
    ...
    const fetchAllProgrammingQuestions = async () => {
      try {
        ...
        //If difficulty is not get all programming questions
        const examQuestionsResponse = await axiosInstance.get(
          `/programmingQuestion/getAllProgrammingQuestions?examId=${examId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );

        const examQuestions = examQuestionsResponse.data.map(
          (q) => q.programmingQuestionId
        );
        const initialCheckedState = {};
        response.data.forEach((question) => {
          initialCheckedState[question.programmingQuestionId] =
            examQuestions.includes(question.programmingQuestionId);
        });
  ...
  //Set checked if pq already present
  ...
  const handleAdditionOfSelected = async () => {
    ...
    try {
      const response = await axiosInstance.post(
        `/programmingQuestion/addProgrammingQuestionList?examId=${examId}`,
        fullQuestions,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (response.status === 200) {
  ...
  return (
    <>
      ...
      <div className="flex justify-center px-2 mx-2 py-3">
        ...
        <Link to={`/addProgQuestion/${examId}`} className="mx-2">
      ...
      <div className="flex-row px-2">
        {progQuestions.length > 0 ? (
          progQuestions.map((question) => (
            <div
              key={question.programmingQuestionId}
              className="px-2 border-1 rounded-lg py-2 my-2"
            >
              <Form.Check
                type="checkbox"
                checked={checkedState[question.programmingQuestionId] || false} // Check if the question's checkbox is checked
                onChange={() =>
                  handleCheckboxChange(question.programmingQuestionId)
                } // Handle state change for the specific question
                className="mb-2"
              />
      ...
      <div className="flex justify-content-end">
        <Button
          onClick={handleAdditionOfSelected}
          className="mx-2"
